...
macro_rules! to_upper {
    ($x: expr) => {
        char::from(($x as u8) - ((BITS[1 + $x as usize] & LOWER) >> 1))
    };
}
...
macro_rules! to_lower {
    ($x: expr) => {
        char::from(($x as u8) + (BITS[1 + $x as usize] & UPPER))
    };
}

#[cfg(test)]
mod tests {
    use crate::toolbox::chr::*;
    use crate::{is_alpha, to_lower, to_upper};
    ...
    fn upper() {
        for c in 'a'..'z' {
            assert_eq!(is_alpha!(to_upper!(c), UPPER), true);
        }
    }
    ...
    fn lower() {
        for c in 'A'..'Z' {
            dbg!(to_lower!(c));
            assert_eq!(is_alpha!(to_lower!(c), LOWER), true);
        }
    }
}
